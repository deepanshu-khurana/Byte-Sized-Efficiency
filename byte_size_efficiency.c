...
void allocate_block(UserBlock user_blocks[], size_t block_count) {
    ...
    printf("Enter size (max %d): ", (int)POOL_SIZE);
    scanf("%zu", &size);
...
void* buddy_malloc(size_t size) {
    size += sizeof(BuddyBlock); // metadata overhead
...
void buddy_free(void* ptr) {
    ...
    while (level < MAX_LEVEL - 1 && buddy->is_free && buddy->level == level) {
        ...
        block->level++;
        buddy = (BuddyBlock*)((char*)pool_start + (((uintptr_t)block - (uintptr_t)pool_start) ^ (MIN_BLOCK_SIZE << block->level)));
...
void allocator_print_stats(UserBlock user_blocks[], size_t block_count) {
    ...
    for (int i = 0; i < MAX_LEVEL; i++) {
        ...
        // Count used blocks: For each block on the pool, check if it's split, has correct level, and is allocated
        // But only if block->level is set (not default/ghost), and block is not free
        ...
        for (uintptr_t addr = pool_base; addr + block_size <= pool_end; addr += block_size) {
            ...
            // Only count if the block has been split to this level and allocated
            if (block->level == i && block->is_free == 0) {
                used_blocks++;
            }
        }

        // If no blocks have been split/allocated at this level, used_blocks should be zero
